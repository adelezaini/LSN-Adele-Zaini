...
//1) External potential and derivatives
//2) Wave functions and derivatives
...
{
  return pow(val,4)-5./2.*val*val;
	/*Armonic potential: double k_elastic = 1;
...
double variationalWaveFunction(double v)
{
  ...
  double mu=0.8125;
  double sigma=0.6125;
  double num1=-(v-mu)*(v-mu);
  double num2=-(v+mu)*(v+mu);
...
double variationalWaveFunction_second(double v)
{
  ...
  double mu=0.8125;
  double sigma=0.6125;
  double m1=(v-mu)*(v-mu)/sigma/sigma;
  double m2=(v+mu)*(v+mu)/sigma/sigma;
