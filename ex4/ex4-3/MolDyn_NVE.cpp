...
/*//Prepare array for measurements iv = 0; //Potential energy ik = 1; //Kinetic energy ie = 2; //Total energy it = 3; //Temperature //n_props = 4; //Number of observables*/
...
/*void Equilibrate(void){
  ...
  cout << "Automatically equilibrate the system" <<endl;
  double temp=nstep;
  nstep=1000;
  for (int i=0;i<3;i++){
  Restart();
  Rescale();
  system("./MolDyn_NVE.exe ");
  }
  nstep=temp;
 autoequi=false;
 restart=true;
